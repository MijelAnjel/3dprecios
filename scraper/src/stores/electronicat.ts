...
// ──────────────────────────────────────────────────────────────
// Electronicat — electronicat.cl — WooCommerce Store API
...
export async function scrapeElectronicat(store: StoreConfig): Promise<ScraperResult[]> {
  ...
  console.log(`[Electronicat] Total productos 3D: ${results.length}`);
  return results;
}
